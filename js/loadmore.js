jQuery(function ($) {
    $('.jobs_loadmore').click(function () {

        let button = $(this),
            data = {
                'action': 'jobs_loadmore',
                'query': jobs_loadmore_params.jobs, // that's how we get params from wp_localize_script() function
                'page': jobs_loadmore_params.current_page
            };

        $.ajax({ // you can also use $.post here
            url: jobs_loadmore_params.ajaxurl, // AJAX handler
            data: data,
            type: 'POST',
            beforeSend: function (xhr) {
                button.text(button.data('loading')); // change the button text, you can also add a preloader image
            },
            success: function (data) {
                if (data) {
                    $('.jobs-regular-list .job-listing-row').append(data); // insert new jobs
                    button.text(button.data('load'));

                    jobs_loadmore_params.current_page++;

                    if (jobs_loadmore_params.current_page == jobs_loadmore_params.max_page) {
                        button.remove(); // if last page, remove the button
                    }

                    // you can also fire the "post-load" event here if you use a plugin that requires it
                    $(document.body).trigger('post-load');
                } else {
                    button.remove(); // if no data, remove the button as well
                }
            }
        });
    });
});